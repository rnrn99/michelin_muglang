...
col.find_one_and_update({"name":michelin["Name"][0]}, {"$set":{"placeId":"ChIJnyme_LaTr0cRzOM_pD32_Ko"}})
col.find_one_and_update({"name":michelin["Name"][1]}, {"$set":{"placeId":"ChIJY8mKQvyOsUcRA-H7nvNI-L8"}})
col.find_one_and_update({"name":michelin["Name"][2]}, {"$set":{"placeId":"ChIJDaXFaJWOnUcRqxYKSDxOfVM"}})
col.find_one_and_update({"name":michelin["Name"][53]}, {"$set":{"placeId":"ChIJ0bNVZ4mjfDURR0LU3v8UBpk"}})

noPlaceId = [["index", "name", "address"]]
...
for idx in noPlaceIdIndex:
    noPlaceId.append([idx, michelin["Name"][idx], michelin["Address"][idx]])
